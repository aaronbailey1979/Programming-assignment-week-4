...
and activity) of each mean-measurement variable.
...
3) Merging these more descriptively labled sets (still labelled). In this
...
and activity level) that are not mean-calculations, and group them by
each combination of subject and activity level (the data set "cut_down").

5) Finally, we compute the correspoding grouped averages, and label the
